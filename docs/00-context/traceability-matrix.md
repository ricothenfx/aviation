...
| 7 | Docker containerisation | Compose profiles (`infra`/`turnaround`/`evidence`); production web image (`next build` + `next start`) validated under load | `infra/compose`, `infra/observability` (ADR-0007), `infra/deploy` (ADR-0006) | compose smoke in CI; ×10 load gate (load-report-f5.md) | F1 done; prod-image shape + deploy structure F5 done |
