 ...
 * cookie name are rebook-ai-specific: the sibling apps share the localhost
 * cookie jar, so names must never collide (rebook-ai architecture.md §5).
 */

export const SESSION_ISSUER = "rebook-ai";
export const SESSION_TTL_SECONDS = 8 * 60 * 60;

/** httpOnly session cookie name; override via REBOOK_AUTH_COOKIE_NAME. */
export const SESSION_COOKIE_NAME = process.env.REBOOK_AUTH_COOKIE_NAME ?? "rbo_session";

const claimsSchema = z.object({
  sub: z.string().uuid(),
  email: z.string().email(),
  name: z.string().min(1),
  role: z.enum(["passenger", "agent", "supervisor"]),
});
...
export function getSessionSecret(): string {
  const secret = process.env.REBOOK_AUTH_SECRET ?? process.env.AUTH_SECRET;
  if (!secret || secret.length < 16) {
    throw new Error(
      "REBOOK_AUTH_SECRET (or AUTH_SECRET) is missing or too short (min 16 chars). " +
