 ...
 * per rebook-ai PRD: supervisor > agent > passenger.
 */
const DEMO_ACCOUNTS = [
  { role: "supervisor", email: "grace.tan@nx-sim.example", password: "supervisor-nx-01" },
  { role: "agent", email: "amir.hassan@nx-sim.example", password: "agent-nx-01" },
  { role: "passenger", email: "nadia.cho@pax-sim.example", password: "passenger-nx-01" },
] as const;
...
function LoginForm() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.ok) {
        // replace(): the landing pages are dynamic so they always render fresh
    ...
    <div className="flex min-h-[calc(100dvh-2rem)] items-center justify-center px-4 py-10">
      <div className="w-full max-w-sm">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-fg">Rebook.ai</h1>
            <p className="text-xs text-muted">
              Passenger disruption concierge · NX NordicX (fictional)
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div className="mt-4">
            ...
            <Input
              ...
              placeholder="you@pax-sim.example"
            />
          </div>
          <div className="mt-3">
        ...
        <div className="mt-4 rounded-lg border border-border bg-surface/60 p-4">
          ...
          <p className="mt-2 text-[11px] leading-4 text-muted">
            ...
            <span className="font-mono">pnpm seed:rebook</span>).
          </p>
        </div>
      </div>
    </div>
